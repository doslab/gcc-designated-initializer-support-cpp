...
process_init_constructor_array (tree type, tree init)
{
  ...
  for (i = 0; VEC_iterate (constructor_elt, v, i, ce); ++i)
    {
  /* If designated initialiser then dont set the index , let it be what it comes from the user*/    
	if (ce->index)
	{
	  gcc_assert (TREE_CODE (ce->index) == INTEGER_CST);
        }
  /* If not a designated initialiser then  set the index explicitly */	
	else
	{
	ce->index = size_int (i);
	}
...
process_init_constructor_record (tree type, tree init)
{
  ...
  for (field = TYPE_FIELDS (type); field; field = TREE_CHAIN (field))
	...
	for(idx=0;idx<VEC_length (constructor_elt, CONSTRUCTOR_ELTS (init));idx++)
	{
		constructor_elt *ce = VEC_index (constructor_elt,CONSTRUCTOR_ELTS (init), idx);
		if(field!=ce->index){
		//printf("\ncoming");
		continue;
		}
	...
	if(idx==VEC_length (constructor_elt, CONSTRUCTOR_ELTS (init)))
	{
	  ...
	  /* If this type needs constructors run for
		...
		if(TYPE_NEEDS_CONSTRUCTING (TREE_TYPE (field)))
		{
