  ...
  while (true)
    {
      ...
      tree identifier_end;
      tree initializer;
      tree start_index, end_index;
      //tree struct_field_start_index;
      //tree struct_field_end_index;	
      bool clause_non_constant_p;
      bool range_initializer = false;
      //bool struct_array_initializer=false;		
	...
	// eg: int a[3]={ [0 ... 2]=4};
		...
		&& cp_lexer_peek_nth_token (parser->lexer, 1)->type == CPP_OPEN_SQUARE
		&& cp_lexer_peek_nth_token (parser->lexer, 3)->type == CPP_ELLIPSIS ) 
	       {
		bool non_constant_p;

		range_initializer = true;		
		cp_lexer_consume_token(parser->lexer);  // consuming '['
		start_index =cp_parser_constant_expression (parser, false, NULL);//cp_lexer_consume_token (parser->lexer)->u.value; 	
		cp_lexer_consume_token(parser->lexer);  // consuming '...'
		end_index = cp_parser_constant_expression (parser, false, NULL);//cp_lexer_consume_token (parser->lexer)->u.value;
		cp_lexer_consume_token(parser->lexer);  // consuming ']'
		cp_lexer_consume_token(parser->lexer);  // consuming '='
	...
	*non_constant_p = true;
      ...
      if(range_initializer)
	{
	 unsigned int i;
	 tree new_identifier;
	 tree temp;
	  if(TREE_CODE(identifier)==INTEGER_CST && TREE_CODE(identifier_end)==INTEGER_CST)
	   {
	         for(i=TREE_INT_CST_LOW(identifier);i<=TREE_INT_CST_LOW(identifier_end);i++)
		   {
		     new_identifier=build_int_cst(temp,i);
		     CONSTRUCTOR_APPEND_ELT(v, new_identifier, initializer);
		   }
	   }
	  else
	   {
	    identifier    =error_mark_node;
	    identifier_end=error_mark_node;
	    error ("both range indexes are not constant");
           }  
